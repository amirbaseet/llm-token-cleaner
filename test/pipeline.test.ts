...
// Import via barrel to cover index.ts re-exports
...
describe("input validation", () => {
  it("clean() throws TypeError on non-string input", () => {
    expect(() => clean(null as unknown as string)).toThrow(TypeError);
    expect(() => clean(undefined as unknown as string)).toThrow(TypeError);
    expect(() => clean(42 as unknown as string)).toThrow(TypeError);
  });

  it("clean() throws TypeError on invalid preset name", () => {
    expect(() => clean("text", { preset: "invalid" as any })).toThrow(
      TypeError,
    );
    expect(() => clean("text", { preset: "invalid" as any })).toThrow(
      /Unknown preset/,
...
describe("barrel export (index.ts)", () => {
  it("re-exports all public API members", () => {
    expect(barrel.clean).toBe(clean);
    expect(barrel.cleanText).toBe(cleanText);
    expect(barrel.stream).toBe(stream);
    expect(barrel.estimateTokens).toBeTypeOf("function");
    expect(barrel.STAGES).toBeDefined();
    expect(barrel.STAGE_IDS).toBeDefined();
    expect(barrel.PRESETS).toBeDefined();
  });
});
